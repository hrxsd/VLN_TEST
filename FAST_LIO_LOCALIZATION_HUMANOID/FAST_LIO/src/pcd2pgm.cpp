...
int main(int argc, char** argv)
{
   ...
   private_nh.param("file_directory", file_directory, std::string("/home/unitree/ws_loc/src/FAST_LIO_LOCALIZATION_HUMANOID/FAST_LIO/PCD/"));
  ...
  //  int size_pcd = pcd_cloud->points.size();
  //  ROS_LOG_STREAM("初始点云数据点数：" << size_pcd);
...
void PassThroughFilter(const double &thre_low, const double &thre_high, const bool &flag_in)
{
    ...
    // int size_pcd = cloud_after_PassThrough->points.size();
    // ROS_LOG_STREAM("直通滤波后点云数据点数：" << size_pcd);
}
