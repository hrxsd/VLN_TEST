        ...
        try:
            messages = [{
                ...
                'content': [
                    ...
                    {
                        'text': '''You are a five-star agent who can understand the current perspective picture. Please analyze this image and provide the following information in JSON format:
...
2. Then, describe what you see in the main content of the image (objects, people, activities, environment) from the first point of view, and formatted as “I saw [the main content of the image] at [time] at [position]”.
...
Focus carefully on reading the text in the top right corner for accurate time and position data.'''
    ...
    def generate_embedding(self, caption: str, image_path: str) -> Optional[List[float]]:
        ...
        try:
            ...
            # 由于DashScope可能不直接支持嵌入生成，这里提供一个模拟实现
            # 实际应用中需要使用专门的嵌入模型
            
            # 模拟嵌入向量（512维）
            np.random.seed(hash(caption + image_path) % 2**32)
