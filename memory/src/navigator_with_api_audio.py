...
# -*- coding: utf-8 -*-
"""
Vector Retrieval ROS System with Audio - Main Entry Point
结合大语言模型、向量数据库和机器人语音播报的主程序
"""
...
class VectorRetrievalSystemWithAudio:
    """向量检索系统主类，集成语音播报"""

    def __init__(self, config_path: str, network_interface: str):
        """初始化系统"""
        # 加载配置
        ...
        rospy.loginfo("Vector Retrieval ROS System with Audio Starting...")

        # 初始化各个组件
        self._init_components(network_interface)

    def _init_components(self, network_interface: str):
        """初始化系统组件"""
        # 数据加载器
        ...
        try:
            ...
            rospy.loginfo(f"AudioClient initialized on interface: {network_interface}")
        except Exception as e:
            rospy.logerr(f"AudioClient initialization failed: {e}")
    ...
    def _speak_answer(self, answer: Dict):
        """用机器人语音播报答案文本"""
        if self.audio_client is not None and "text" in answer:
            try:
                text = answer["text"]
                if text:
                    self.audio_client.TtsMaker(text, 0)
            except Exception as e:
                rospy.logerr(f"AudioClient TtsMaker failed: {e}")

    def run(self):
        """运行主循环"""
        print("=== ROS向量数据库检索系统（带语音播报） ===")
        print("命令说明:")
        print("- 输入问题进行查询")
        print("- 'cancel': 取消导航")
        print("- 'status': 查看状态")
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        config_path = sys.argv[1]
        network_interface = "enp3s0"  # 默认接口名，可根据实际情况修改
    else:
        config_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)),
            'config/config.yaml'
        )
        network_interface = "enp3s0"  # 默认接口名
    ...
    try:
        # 创建并运行系统
        system = VectorRetrievalSystemWithAudio(config_path, network_interface)
        system.run()
